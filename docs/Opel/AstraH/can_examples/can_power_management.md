...
ID: 0x170
DLC: 3 байта
Data[0]: Key State (состояние ключа) ← ЧИТАЕТСЯ ПРИБОРКОЙ!
Data[1]: 0x55 (константа)
Data[2]: 0x55 (константа)
```

**ВАЖНО:** Приборка читает состояние из Data[0], а не Data[2]! Неправильный формат вызывает перезагрузку приборки!
